...
PigBotBasePaceRace: A bot with min score, max behind, + when to go nuts. (PigBot)
"""
...
Game/Bot Programming: "Ichabod" O'Brien
Bot Design: Roger Johnson, Reiner Knizia, Todd Neller, "Ichabod" O'Brien, 
    Clifton Presser
"""
...
class PigBotBasePaceRace(PigBot):
    ...
    def __init__(self, base = 19, pace = 14, race = 31, taken_names = []):
        ...
        taken_names: Names already used by a player. (list of str)
        """
        super(PigBotValue, self).__init__(taken_names, 'b')
        self.base = base
        self.pace = pace
        self.race = race
...
class PigBotPaceRace(PigBot):
    """
    A Pig bot with values for min score, behind modifier, and when to go nuts. (PigBot)
    ...
    parameters = ['minimum score', 'behind modifier', 'when to go for it']
    ...
    def __init__(self, pace = 21, modifier = 8, race = 29, taken_names = []):
        ...
        taken_names: Names already used by a player. (list of str)
        """
        super(PigBotValue, self).__init__(taken_names, 'p')
        self.pace = pace
    ...
    def ask(self, prompt):
        ...
        # Roll until the base is met.
        if turn_score < hold_value or my_score + turn_score > 100:
            return 'roll'
        # Roll to win if someone is within race.
...
class PigBotRolls(PigBot):
    ...
    def __init__(self, rolls = 5, taken_names = []):
        ...
        taken_names: Names already used by a player. (list of str)
        """
        super(PigBotValue, self).__init__(taken_names, 'r')
...
class PigBotScoringTurns(PigBot):
    ...
    def __init__(self, scoring_turns = 4, taken_names = []):
        ...
        taken_names: Names already used by a player. (list of str)
        """
        super(PigBotValue, self).__init__(taken_names, 't')
...
class Pig(game.Game):
    ...
    def handle_options(self):
        ...
        # !! refactor into parse_options vs. ask_options
        # Set the defaults.
        ...
        if self.raw_options:
            # Handle options from play command.
